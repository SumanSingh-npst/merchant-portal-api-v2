...
export class ReconService {
  ...
  async initiateTwoWayRecon() {
    ...
    try {
      ...
      return {
        status: true,
        msg: 'Reconciliation process completed successfully',
      };
    } catch (error) {
      this.logger.error('Error during reconciliation process', error);
      return {
        status: false,
        msg: 'Reconciliation process failed',
        error: error,
      };
    }
  }
  ...
  async executeQuery(query: string): Promise<number> {
    ...
    while (attempt < maxAttempts) {
      try {
        ...
        this.logger.error(
          `Attempt ${attempt} failed. Error: ${error.message}`,
          error,
        );
        ...
        if (attempt < maxAttempts) {
          ...
          this.logger.error(
            `All ${maxAttempts} attempts failed. query ${query} could not be executed.`,
            error,
          );
