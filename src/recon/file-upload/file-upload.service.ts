...
export class FileUploadService {
  ...
  private createFileUploadResponse(
    file: Multer.File,
    isSwitch: boolean,
    isDuplicate: boolean,
    count: number,
  ...
  async validateAndStoreFiles(files: Multer.File[], isSwitchFile: boolean) {
    try {
      ...
      const nonDuplicateFiles = files.filter(
        (file) =>
          !response.some(
            (res) => res.fileName === file.originalname && res.isDuplicate,
          ),
      );
      if (nonDuplicateFiles.length === 0) {
        return {
          status: false,
          msg: 'some files are already uploaded into the portal earlier. remove them and reupload',
        };
      }
      ...
      return {
        status: true,
        duplicateFiles: response,
        msg: `File processing took ${processingTime} minutes`,
      };
    } catch (error) {
  ...
  private async processFiles(
    nonDuplicateFiles: Multer.File[],
    isSwitchFile: boolean,
  ) {
    const fileProcessingPromises = this.fileUploadHistoryData.map(
      (fileRecord) => {
        const file = nonDuplicateFiles.find(
          (f) => f.originalname === fileRecord.fileName,
        );
        if (file) {
    ...
    return new Promise<void>((resolve, reject) => {
      fs.createReadStream(filePath)
        ...
        .on('end', () => {
          ...
          const fileRecord = this.fileUploadHistoryData.find(
  ...
  private async storeDataToDatabase(isSwitchFile: boolean) {
    ...
    const tmpDuplicates = this.validTXNS.filter((txn) =>
      existingIds.includes(txn['UPI_TXN_ID']),
    );
    ...
    this.validTXNS = this.validTXNS.filter(
      (txn) => !existingIds.includes(txn['UPI_TXN_ID']),
    );
    ...
    await this.dbSvc.insertJunkDataToDB(
      this.invalidTXNS,
      isSwitchFile,
      'INVALID_TXN',
    );
    ...
    try {
      ...
      fileType === 'SWITCH'
        ? await this.clickdb.exec({
            query: `DELETE FROM SWITCH_TXN WHERE UPLOAD_ID = '${uploadId}';`,
          })
        : await this.clickdb.exec({
            query: `DELETE FROM NPCI_TXN WHERE UPLOAD_ID = '${uploadId}';`,
          });
