...
export class ReportController {
    constructor(private readonly reportSvc: ReportService, private zipService: ZipService) {
    }
    ...
    async downloadTXNS(@Body() body: DownloadRequestDTO, @Res() res: Response) {
        ...
        const totalRecords = await this.reportSvc.getAllTXNCount(body.startDate, body.endDate, body.txnType);
        await this.reportSvc.processChunks(body.startDate, body.endDate, body.txnType, chunkSize, totalRecords);
        this.zipService.finalizeZip();

        // Stream the ZIP file to the client
        res.setHeader('Content-Type', 'application/zip');
        res.setHeader('Content-Disposition', 'attachment; filename=transactions.zip');
